...
1. In `/config/app.php`, add the following to `providers`:
  ...
  ```
  Bitw\Recaptcha\RecaptchaServiceProvider::class,
  ```
  and the following to `aliases`:
  ```
  'Recaptcha' => Bitw\Recaptcha\Facades\Recaptcha::class,
  ```
2. Run `php artisan vendor:publish --provider="Bitw\Recaptcha\RecaptchaServiceProvider"`.
